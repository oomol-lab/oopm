...
describe.sequential("install all", () => {
    it("should success", async (ctx) => {
        ...
        // publish `remote_storage` to registry
        {
            ...
            await publish(path.join(remoteStorage, "e-0.0.1"), ctx.registry.endpoint, "fake-token");
            await publish(path.join(remoteStorage, "e-0.0.2"), ctx.registry.endpoint, "fake-token");
            await publish(path.join(remoteStorage, "e-0.0.3"), ctx.registry.endpoint, "fake-token");
        }
    ...
    it("should install missing deps when exec integrity check", async (ctx) => {
        ...
        // publish `remote_storage` to registry
        {
            ...
            await publish(path.join(remoteStorage, "d-0.0.1"), ctx.registry.endpoint, "fake-token");
            await publish(path.join(remoteStorage, "d-0.0.2"), ctx.registry.endpoint, "fake-token");
        }
...
describe.sequential("install deps", () => {
    ...
    it("manually specifying the version should be the highest priority", async (ctx) => {
        ...
        {
            const remoteStorage = path.join(p, "remote_storage");
            await publish(path.join(remoteStorage, "a-0.0.1"), ctx.registry.endpoint, "fake-token");
            await publish(path.join(remoteStorage, "a-0.0.2"), ctx.registry.endpoint, "fake-token");
            await publish(path.join(remoteStorage, "b-0.0.1"), ctx.registry.endpoint, "fake-token");
            await publish(path.join(remoteStorage, "b-0.0.2"), ctx.registry.endpoint, "fake-token");
        }
